...
// This is an example to help debug the get_matched_basis_vector function.
...
fn main() {
    ...
    // ----------------------------------------------------------------------------------
    // Get all d1 basis vectors 
    // ----------------------------------------------------------------------------------
    //
  
    println!("");
    println!("\nCalculating all d1 basis vectors\n");

    let simplices = vec![   exhact::clique::Simplex{ filvalue: 1, vertices: vec![0, 1] } ,
                            exhact::clique::Simplex{ filvalue: 2, vertices: vec![0, 2] } ,
                            exhact::clique::Simplex{ filvalue: 1, vertices: vec![0, 3] } ,
                            exhact::clique::Simplex{ filvalue: 1, vertices: vec![1, 2] } ,
                            exhact::clique::Simplex{ filvalue: 2, vertices: vec![1, 3] } ,
                            exhact::clique::Simplex{ filvalue: 1, vertices: vec![2, 3] }    ];

    for simplex in simplices {
        println!("Basis vector for {:?}:", &simplex);
        for sparsepair in factored_complex.get_matched_basis_vector(1, &simplex) {
            println!("{:?}", sparsepair );
